...
typedef struct { float3 *position; float3 *velocity; } Body;

void randomizeBodies(float3 *position, float3 *velocity, int n) {
    for (int i = 0; i < n; i++) {
...
__global__ void bodyForce(Body p, float dt, int n) {
    ...
    for (int j = 0; j < n; j++) {
        float dx = p.position[j].x - px;
        float dy = p.position[j].y - py;
        float dz = p.position[j].z - pz;
...
int main(const int argc, const char **argv) {
    ...
    int bytes = nBodies * sizeof(float3);
    float3 *position_h = (float3 *)malloc(bytes);
    float3 *velocity_h = (float3 *)malloc(bytes);
    Body p = { position_h, velocity_h };

    randomizeBodies(position_h, velocity_h, nBodies);

    float3 *position_d, *velocity_d;
