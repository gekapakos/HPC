...
PGM_IMG contrast_enhancement_GPU(PGM_IMG img_in) {
    ...
    err = cudaHostAlloc((void**)&img_in.img, img_in.w * img_in.h * sizeof(unsigned char), cudaHostAllocMapped);
    if (err != cudaSuccess) {
        fprintf(stderr, "CUDA Host Alloc error: %s\n", cudaGetErrorString(err));
        if (img_in.img) cudaFreeHost(img_in.img);
        return(img_in);
    }
    ...
    err = cudaMalloc((void **)&img_gpu.img, img_gpu.w * img_gpu.h * sizeof(unsigned char));
    if (err != cudaSuccess) {
    ...
    histogramGPU<<<((img_cpu.w*img_cpu.h)/256), 256, 256*sizeof(int) >>>(d_hist, d_ImgIn, img_cpu.w, img_cpu.h);
    ...
    histogram_equalization_GPU<<<((img_cpu.w * img_cpu.h)/256), 256, 256*sizeof(int)>>>(img_gpu.img, d_ImgIn, d_lut, img_cpu.w, img_cpu.h);
